...
class Updates
{
	...
	private function _getUpdates()
	{
		if (!$this->isFocus()) {
			...
			if (!empty($this->aResponse)) {
				if ($this->aResponse['status'] == 'error') {
					$this->aPlugins = [];
					$this->aTheme = [];
					$this->errMsg = isset($this->aResponse['msg']) ? $this->aResponse['msg'] : '';
				} else {
		...
		$this->responseCode = isset($this->aResponse['code']) ? $this->aResponse['code'] : 'OKE';
		$this->warningMsg = isset($this->aResponse['warningMsg']) ? $this->aResponse['warningMsg'] : '';
		if ($this->aResponse['status'] == 'success') {
			$aRawPlugins = $this->aResponse['aPlugins'];
			update_option('wilcity_plugins', $aRawPlugins);
	...
	public function showUpTheme()
	{
        ...
        <div id="wilcity-update-theme" class="ui segment" style="margin-top: 30px;">
			...
			<?php if (!empty($this->warningMsg)) : ?>
                <p class="ui message error positive"><?php echo $this->warningMsg; ?></p>
			<?php endif; ?>
